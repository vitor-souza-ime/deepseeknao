...
class NAODeepSeekChat:
    def __init__(self, nao_ip="172.15.3.253", nao_port=9559, model_name="deepseek-llm:latest", tts_volume=0.1):
        """
        Chat NAO + DeepSeek LLM local via subprocess
        """
        try:
    ...
    def check_ollama_connection(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if "deepseek-llm" not in models_output:
                    print(f"\n⚠️ Modelo DeepSeek não encontrado!")
                    print(f"💡 Execute: ollama pull deepseek-llm:latest")
    ...
    def ask_deepseek(self, question: str):
        """
        Processa pergunta com DeepSeek via subprocess e mede o tempo
        """
        try:
            print(f"🧠 DeepSeek ({self.model_name}) processando: '{question}'")
            
            # ⏱️ INÍCIO DA MEDIÇÃO
            start_time = time.time()
            
            # Prompt otimizado para DeepSeek
            # DeepSeek responde bem a instruções diretas e estruturadas
            prompt = f"""You are a helpful robot assistant. Answer briefly in 1-2 clear sentences.

Question: {question}

Answer:"""
            
            # Chama o modelo DeepSeek via ollama CLI
            result = subprocess.run(
            ...
            if result.returncode == 0:
                ...
                prefixes_to_remove = ["Assistant:", "Answer:", "A:", "Response:"]
                for prefix in prefixes_to_remove:
                    if resposta.startswith(prefix):
                        resposta = resposta[len(prefix):].strip()
                
                # Limpa tokens especiais comuns em LLMs
                tokens_to_clean = ["<|im_end|>", "<|im_start|>", "<|endoftext|>", "<|user|>", "<|assistant|>"]
                for token in tokens_to_clean:
                    resposta = resposta.replace(token, "")
                ...
                # Limita a 1-2 frases (DeepSeek tende a ser conciso)
                ...
                print(f"   └─ Ratio: {len(resposta)/len(question)}")
                print(f"   └─ Tokens/segundo: ~{len(words) / elapsed_time:.1f} palavras/s")
                print(f"🤖 DeepSeek respondeu: '{resposta}'\n")
                
                return resposta
            else:
                elapsed_time = time.time() - start_time
                error_msg = result.stderr.strip() if result.stderr else "Erro desconhecido"
                print(f"❌ Erro ao executar DeepSeek após {elapsed_time:.2f}s:")
                print(f"   {error_msg}")
            ...
            elapsed_time = time.time() - start_time
            print(f"❌ Erro com DeepSeek após {elapsed_time:.2f}s: {e}")
    ...
    def run_chat(self):
        ...
        # Saudação
        self.speak("Hello! I'm NAO with DeepSeek AI running locally. I'm ready to chat with you in English. Please speak clearly.")
        ...
        while True:
            try:
                ...
                print("🤖 Processando com DeepSeek...")
...
def main():
    ...
    try:
        print("=" * 60)
        print("🤖 NAO + DeepSeek LLM Chat (CLI via subprocess)")
        print("=" * 60)
        ...
        chat = NAODeepSeekChat(NAO_IP, model_name=MODEL_NAME)
        chat.run_chat()
        ...
        print("4. ✓ DeepSeek está instalado (ollama pull deepseek-llm:latest)")
